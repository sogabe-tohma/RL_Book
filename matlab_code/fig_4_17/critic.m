classdef critic <handle
    properties(SetAccess=public)
        c_w={unform( 2,300),unform(300,600),unform(1,600),unform(600,600),random('Uniform',-3e-3, 3e-3, [600,1])};
        c_b={zeros(1,300),zeros(1,600),zeros(1,600),zeros(1,600),zeros(1)};
        ct_w={unform( 2,300),unform(300,600),unform(1,600),unform(600,600),random('Uniform',-3e-3, 3e-3, [600,1])};
        ct_b={zeros(1,300),zeros(1,600),zeros(1,600),zeros(1,600),zeros(1)};
    ...
    methods    
        ...
        function [loss_action]=evaluate_action_loss(self,X_s,X_a,Y_tgt,tar)
            if tar==1
                Wc1 = self.c_w{1};
                bc1 = self.c_b{1};
                Wc2_s = self.c_w{2};
            ...
            c_H3=max(0,c_z3);
            coutput=(c_H3*Wc4)+bc4;
            errors=abs(coutput-Y_tgt);
            grand_out=errors;
            c_out1=(grand_out*Wc4');
            c_out1(c_z3<=0)=0;
            c_out2=(c_out1*Wc3');
            loss_action=(c_out2*Wc2_a');                    
        end
        function y= rel(X) 
            y=tanh(X);
        end
