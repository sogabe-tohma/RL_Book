...
num_actions=4
class Qmaze():
    def __init__(self, maze, rat=(2,3)):
        self.rat=rat
        self.actions_dict = {
            0: ['LEFT','left'],
            1: ['UP','up'],
        ...
        self.N_goal=[(r,c) for r in range(nrows) for c in range(ncols) if self._maze[r,c] == -1.0][0]
    ...
    def get_reward(self):
        #reward for goal is 1 if blocked -0.5, if invalid -0.7 and valid is -0.04
    ...
    def observe(self):
        #makes the current state to flatt
        canvas = self.draw_env()
        envstate=[]
        nrows, ncols = canvas.shape
        # state=total space with can place with 1 and cannot with 0 and rat place with 0.5
